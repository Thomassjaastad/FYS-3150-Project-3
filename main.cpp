...
int main(int numArguments, char **arguments)
{
  ...
  /*  double r;
    double r_prev = 1000.; //Want this to be at a minimum and pull out angle from y and x component
   ...
   */
    ...
    /*    r = sqrt(bodies[1].position.x()*bodies[1].position.x() + bodies[1].position.y()*bodies[1].position.y());
        ...
        if ((r > r_prev) && (r_prevprev > r_prev)) {
            ...
            theta = atan2(y_prev,x_prev)*57;
            cout << mercury_yr  <<  " " << setprecision(16) <<  theta << " " << r_prev << endl;
            mercury_yr++;
        }
